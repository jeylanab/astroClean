...
const DetachedFlow = ({ propertyInfo, onBack }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 animate-in fade-in slide-in-from-right-8 duration-500 text-white">

      {/* Step 2: Bedrooms */}
      {step === 2 && (
        <div className="flex flex-col h-full animate-in fade-in duration-300">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-6 leading-tight text-white">Number of bedrooms?*</h2>
      ...
      {step === 2.5 && (
        <div className="flex flex-col h-full animate-in zoom-in-95 duration-500">
          <div className="mb-8">
            ...
            <h4 className="text-sm font-bold text-white/50 leading-relaxed tracking-tight italic">
          ...
          <div className="flex flex-col gap-3 max-w-md">
            <button
              onClick={() => setStep(13)}
              className="w-full p-5 rounded-2xl border-2 border-yellow-400 bg-yellow-400/10 hover:bg-yellow-400 group transition-all text-left flex gap-4 items-center"
            >
            ...
            <button
              onClick={onBack}
              className="w-full p-5 rounded-2xl border-2 border-white/10 bg-white/5 hover:border-red-400 transition-all text-left group flex gap-4 items-center"
            >
      ...
      {step === 3 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-6 text-white leading-tight">Has an extension?*</h2>
          <div className="flex items-start w-full py-4 mb-4">
            <img src={extentionImg} className="w-40 md:w-56 h-auto drop-shadow-2xl" alt="" />
          </div>
          <div className="flex flex-col gap-2 w-full max-w-sm">
            <button onClick={() => { setFormData({ ...formData, hasExtension: true }); next(); }} className={btnPrimary}>Yes, I have one</button>
            <button onClick={() => { setFormData({ ...formData, hasExtension: false }); setStep(5); }} className={btnSecondary}>No extension</button>
          </div>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-1 text-white">Extension Type?*</h2>
          <p className="text-[10px] font-bold text-yellow-400 uppercase mb-6 tracking-widest">Select multiple if needed</p>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 w-full flex-1">
            {[{ id: 'loft', title: 'Loft', img: loftImg }, { id: 'side', title: 'Side', img: sideImg }, { id: 'rear', title: 'Rear', img: rearImg }].map((item) => (
              <button key={item.id} onClick={() => toggleExtension(item.id)}
                className={`p-4 rounded-2xl border transition-all flex flex-col items-center justify-center ${formData.extensionTypes.includes(item.id) ? 'border-yellow-400 bg-yellow-400/10 shadow-lg shadow-yellow-400/10' : 'border-white/10 bg-white/5'}`}>
          ...
          <button onClick={next} className={`${confirmBtn} mt-6`}>Continue</button>
        </div>
      )}
      ...
      {step === 5 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-6 text-white leading-tight">Has a skylantern?*</h2>
          <div className="flex items-start w-full py-4 mb-4">
            <img src={skylanternImg} className="w-40 md:w-56 h-auto drop-shadow-2xl" alt="" />
          </div>
          <div className="flex flex-col gap-2 w-full max-w-sm">
            <button onClick={() => { setFormData({ ...formData, hasSkylantern: true }); next(); }} className={btnPrimary}>Yes</button>
            <button onClick={() => { setFormData({ ...formData, hasSkylantern: false }); setStep(7); }} className={btnSecondary}>No</button>
          </div>
        </div>
      )}
      ...
      {step === 6 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-6 leading-tight text-white">Quantity?*</h2>
          <div className="grid grid-cols-2 gap-2 max-w-md w-full">
            {['1', '2', '3', '4+'].map((opt, i) => (
              <button key={opt} onClick={() => { setFormData({ ...formData, skylanternCount: opt }); next(); }}
                className="p-4 rounded-xl border border-white/10 bg-white/5 hover:border-yellow-400 hover:bg-yellow-400/10 transition-all font-bold text-center group">
      ...
      {step === 7 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-6 text-white leading-tight">Has a conservatory?*</h2>
          <div className="flex items-start w-full py-4 mb-4">
            <img src={conservatoryImg} className="w-40 md:w-56 h-auto drop-shadow-2xl" alt="" />
          </div>
          <div className="flex flex-col gap-2 w-full max-w-sm">
            <button onClick={() => { setFormData({ ...formData, hasConservatory: true }); next(); }} className={btnPrimary}>Yes</button>
            <button onClick={() => { setFormData({ ...formData, hasConservatory: false }); setStep(9); }} className={btnSecondary}>No</button>
          </div>
        </div>
      )}
      ...
      {step === 8 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-2 text-white">Glass panels?*</h2>
          <p className="text-[10px] font-bold text-white/40 mb-8 max-w-xs uppercase">Floor to ceiling sections only.</p>
          <input
            type="number"
            placeholder="10"
            className={`${inputBase} text-4xl max-w-[120px] mb-8`}
            onChange={(e) => setFormData({ ...formData, conservatoryPanels: e.target.value })}
          />
          <button onClick={next} className={confirmBtn}>Confirm</button>
        </div>
      )}
      ...
      {step === 9 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-6 text-white leading-tight">Any velux?*</h2>
          <div className="flex items-start w-full py-4 mb-4">
            <img src={veluxImg} className="w-40 md:w-56 h-auto drop-shadow-2xl" alt="" />
          </div>
          <div className="flex flex-col gap-2 w-full max-w-sm">
            <button onClick={() => { setFormData({ ...formData, hasVelux: true }); next(); }} className={btnPrimary}>Yes</button>
            <button onClick={() => { setFormData({ ...formData, hasVelux: false }); setStep(11); }} className={btnSecondary}>No</button>
          </div>
        </div>
      )}
      ...
      {step === 10 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-2 text-white">Quantity?*</h2>
          <p className="text-[10px] font-bold text-white/40 mb-8 uppercase">Reach from ground level only.</p>
          <input
            type="number"
            placeholder="6"
            className={`${inputBase} text-4xl max-w-[120px] mb-8`}
      ...
      {step === 11 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-8 leading-tight text-white">Rear Access?*</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full flex-1">
            <button onClick={() => { setFormData({ ...formData, rearAccess: true }); next(); }}
              className="p-6 rounded-2xl border border-white/10 bg-white/5 hover:border-yellow-400 hover:bg-yellow-400/10 transition-all flex flex-col items-center group">
      ...
      {step === 12 && (
        <div className="flex flex-col h-full animate-in zoom-in-95 duration-500">
          <h2 className="text-3xl md:text-4xl font-black uppercase mb-1 text-white">Instant Quote</h2>
          <p className="text-[10px] font-bold text-white/40 mb-8 uppercase tracking-widest">Select Frequency</p>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
            {/* Monthly */}
            <div onClick={next} className="bg-yellow-400 p-6 rounded-3xl border-2 border-yellow-400 cursor-pointer hover:shadow-2xl hover:shadow-yellow-400/20 relative group transition-all">
            ...
            {/* 2-Monthly */}
            <div onClick={next} className="bg-white/5 p-6 rounded-3xl border border-white/10 cursor-pointer hover:border-white/30 transition-all group">
      ...
      {step === 13 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-8 leading-tight text-white">Your Details*</h2>
          <div className="grid gap-6 max-w-sm w-full">
            <input type="text" placeholder="First Name*" className={`${inputBase} text-xl`} />
            <input type="tel" placeholder="Phone Number*" className={`${inputBase} text-xl`} />
            <input type="email" placeholder="Email Address" className={`${inputBase} text-xl`} />
            <button onClick={next} className="bg-yellow-400 text-[#010191] py-4 rounded-xl font-black text-lg mt-4 hover:bg-white transition-all uppercase tracking-wide">Next</button>
          </div>
        </div>
      )}
      ...
      {step === 14 && (
        <div className="flex flex-col h-full animate-in fade-in">
          <h2 className="text-2xl md:text-3xl font-black uppercase mb-8 leading-tight text-white">Address*</h2>
          <div className="grid gap-6 max-w-sm w-full">
            <input type="text" placeholder="Address line 1*" className={`${inputBase} text-lg`} />
            <input type="text" placeholder="Postcode*" className={`${inputBase} text-lg`} />
            <button onClick={next} className="bg-yellow-400 text-[#010191] py-4 rounded-xl font-black text-lg mt-6 uppercase hover:bg-white transition-all tracking-wide">Book Now</button>
          </div>
        </div>
      )}
      ...
      {/* Final Step: Success */}
      {step === 15 && (
        <div className="flex flex-col items-start justify-center py-8 space-y-4 animate-in zoom-in duration-700">
          <div className="w-16 h-16 bg-yellow-400 text-[#010191] rounded-full flex items-center justify-center text-2xl shadow-lg shadow-yellow-400/30 animate-bounce font-black">✓</div>
          <h2 className="text-2xl md:text-4xl font-black uppercase leading-none max-w-sm text-white">Confirmed!</h2>
          <p className="text-sm font-bold text-white/40 uppercase">We'll contact you within 24 hours.</p>
        </div>
      )}
